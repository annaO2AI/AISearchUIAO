...
const CompareSOWs = () => {
    const [analysisResult, setAnalysisResult] = useState<ComparisonTable | null>(null)
    ...
    const analyzeFormik = useFormik({
        ...
        onSubmit: async (values) => {
            ...
            try {
                const response = await fetch(API_ROUTES.analyzeSow, {
                    method: "POST",
                    body: formData,
                })
                ...
                if (result.ok && result.comparison_table) {
                    setAnalysisResult(result.comparison_table)
                } else {
        ...
        <div>
            <form onSubmit={analyzeFormik.handleSubmit} className="space-y-4">
                    ...
                    <div className="w-[50%] p-12 bg-white rounded-lg shadow-md border-o2 relative">
                        ...
                        <div className="inputdesign rounded-lg  flex flex-col items-center">
                            ...
                            <label
                                ...
                                className="mb-4 bluebtn-custome block w-full text-base text-gray-500 p-6 mx-auto w-[350px] file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-base file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer"
                            >
                                Choose a file
                            </label>
                            <input
                                    ...
                                    <div key={index} className="flex items-center gap-3 bg-[#3C77EF] p-6 rounded-lg">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <div className="text-sm text-gray-500 flex gap-2 content-center">
                                                ...
                                                {/* <div>
                                                    Path: {fileMetadata[file.name]?.path || "Not available"}
                                                </div> */}
                                            </div>
                                        </div>
                                        <button
                                            ...
                                            onClick={() => removeExistingFile(index)}
                                             className="text-white px-5 py-1 text-lg rounded"
                                        >
                                            ×
                                        </button>
                                    </div>
                    ...
                    <div className="w-[50%] p-12 bg-white rounded-lg shadow-md border-o2">
                        ...
                        <div className="inputdesign-new rounded-lg flex flex-col items-center">
                            ...
                            <label
                                ...
                                className="mb-4 bluebtn-custome-new block w-full text-base text-gray-500 p-6 mx-auto w-[350px] file:mr-4 file:py-2 file:px-4 file:rounded file:border-0 file:text-base file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100 cursor-pointer"
                            >
                                Choose a file
                            </label>
                            <input
                        ...
                        {analyzeFormik.touched.new_file && analyzeFormik.errors.new_file && (
                            <div className="text-red-500 text-base mt-1">
                        ...
                        {analyzeFormik.values.new_file && (
                            <div className="flex items-center gap-3 mt-2 bg-[#8337D4] p-6 rounded-lg">
                                ...
                                <div className="text-sm text-white flex gap-3">
                                    ...
                                    {/* <div>
                                        Path: {fileMetadata[analyzeFormik.values.new_file.name]?.path || "Not available"}
                                    </div> */}
                                </div>
                                </div>
                            </div>
                ...
                <input
                    ...
                    onBlur={analyzeFormik.handleBlur}
                    className="block w-full text-base text-gray-700 border border-gray-300 rounded py-2 px-3"
                ...
                {errorMessage && (
                    <div className="text-red-500 text-base mt-2">{errorMessage}</div>
                )}
                <div className="flex mt-16 mb-12">
                    ...
                    >
                        {analyzeFormik.isSubmitting ? "Comparing..." : "Compare SOWs"}
                    </button>
                </div>
            </form>

            {analysisResult && (
                <div className="mt-8 p-12 rounded-lg border border-gray-200 bg-white shadow-md mb-6">
                    <h3 className="text-lg font-semibold mb-4">{analysisResult.title}</h3>
                    <div className="mb-4 text-base osubtitle">
                        <p><strong>{analysisResult.columns[1]}:</strong> {analysisResult.left_doc}</p>
                        <p><strong>{analysisResult.columns[2]}:</strong> {analysisResult.right_doc}</p>
                    </div>
                    <div className="overflow-x-auto">
                        <table className="min-w-full border-collapse border border-gray-300">
                            <thead>
                                <tr className="bg-gray-100">
                                    {analysisResult.columns.map((column, index) => (
                                        <th
                                            key={index}
                                            className="border border-gray-300 px-4 py-4 text-left text-base font-medium text-gray-700"
                                        >
                                            {column}
                                        </th>
                                    ))}
                            ...
                            <tbody>
                                {analysisResult.rows.map((row, rowIndex) => (
                                    <tr key={rowIndex} className={rowIndex % 2 === 0 ? "bg-white" : "bg-gray-50"}>
                                        <td className="border border-gray-300 px-4 py-4 text-base  font-semibold">
                                            {row.section}
                                        </td>
                                        <td className="border border-gray-300 px-4 py-4 text-base ">
                                            {row.left}
                                        </td>
                                        <td className="border border-gray-300 px-4 py-4 text-base ">
                                            {row.right}
